...
<?php
    $user_id = $place_id = $stime = $ftime ="";
    $user_id_err = $place_id_err = $stime_err = $ftime_err = "";

    if(isset($_POST['edit'])){

        $user_id = mysqli_real_escape_string($db, $_GET['user_id']);
        $place_id = mysqli_real_escape_string($db, $_GET['place_id']);
        $stime = mysqli_real_escape_string($db, $_GET['stime']);
        $ftime = mysqli_real_escape_string($db, $_GET['ftime']);
        $description = mysqli_real_escape_string($db, $_GET['description']);
        $session=$_SESSION['user_id'];

        //validation
          $query = "UPDATE history user_id='$user_id', place_id= '$place_id', stime= '$stime' , ftime ='$ftime', description ='$description', session= '$session'"; 
    ...
    <div class="span6">
      <div class="widget-box">
        ...
        <div class="widget-content nopadding">
          <form action="edit_assign.php" method="GET" class="form-horizontal">
            ...
            <div class="control-group ">
              ...
              <div class="controls">
                <select name="place_id" id="place_id " class="span11">
                  <option value="" disabled selected>--Select Place--</option>
                  <?php 
                    $query = "SELECT * FROM Placement order by kebele";
                    $result = mysqli_query($db, $query);
                    while($row = mysqli_fetch_array($result)){
                      ?>
                        <option value="<?php echo $row['id'];  ?>"><?php echo "Kebele:". $row['kebele'].","." " ."Place:".$row['place']; ?></option>
                      <?php
                    }
                  ?>
